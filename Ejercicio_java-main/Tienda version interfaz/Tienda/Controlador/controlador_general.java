...
public class controlador_general {
    ...
    public String tomar_datos_carro(String marca, String placa, String modelo) {
        ...
        try {
            ...
            // Busca la primera posición libre en la matriz
            carro_modelo[][] matriz = api.getMatrizCarros();
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    if (matriz[i][j] == null) {
                        return api.agregar_carro(nuevo, i, j);
                    }
                }
            }
            return "Error: La matriz de carros está llena (máximo 9 carros).";
        } catch (Exception e) {
            return e.getMessage();
        }
    }
    ...
    public String tomar_datos_motor(String tipo_motor, String numero_serie, String cilindraje, String extra) {
        ...
        try {
            ...
            // Busca la primera posición libre en la matriz
            motor_modelo[][] matriz = api.getMatrizMotores();
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    if (matriz[i][j] == null) {
                        return api.agregar_motor(nuevo, i, j);
                    }
                }
            }
            return "Error: La matriz de motores está llena (máximo 9 motores).";
        } catch (Exception e) {
            return e.getMessage();
        }
    }
